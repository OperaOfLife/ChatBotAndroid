...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val retrofit = Retrofit.Builder()
            .baseUrl("http://172.19.208.1:5000/")
    ...
    private val callBack = object  : Callback<ChatResponse>{
        override fun onResponse(call: Call<ChatResponse>, response: Response<ChatResponse>) {
            if(response.isSuccessful &&  response.body()!= null){
                adapterChatBot.addChatToList(response.body()!!.chatBotReply)
            }else{
                Toast.makeText(this@MainActivity, "Something went wrong here", Toast.LENGTH_LONG).show()
            }
        }
